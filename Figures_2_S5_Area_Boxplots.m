...
%60 min junc
s1_60min_junc = readmatrix("Sample1_60min_j_2.5_37.csv", 'Delimiter', ',');
s2_60min_junc = readmatrix("Sample2_60min_j_2.5_37.csv", 'Delimiter', ',');
s3_60min_junc = readmatrix("Sample3_60min_j_2.5_37.csv", 'Delimiter', ',');

%60 min mid
s1_60min_mid = readmatrix("Sample1_60min_m_2.5_37.csv", 'Delimiter', ',');
s2_60min_mid = readmatrix("Sample2_60min_m_2.5_37.csv", 'Delimiter', ',');
s3_60min_mid = readmatrix("Sample3_60min_m_2.5_37.csv", 'Delimiter', ',');

%60 min tip
s1_60min_tip = readmatrix("Sample1_60min_t_2.5_37.csv", 'Delimiter', ',');
s2_60min_tip = readmatrix("Sample2_60min_t_2.5_37.csv", 'Delimiter', ',');
s3_60min_tip = readmatrix("Sample3_60min_t_2.5_37.csv", 'Delimiter', ',');
...
s1_60min_j = s1_60min_junc(:, 3).*(38.52/546).^2;
s2_60min_j = s2_60min_junc(:, 3).*(38.52/546).^2;
s3_60min_j = s3_60min_junc(:, 3).*(38.52/546).^2;

s1_60min_m = s1_60min_mid(:, 3).*(38.52/546).^2;
s2_60min_m = s2_60min_mid(:, 3).*(38.52/546).^2;
s3_60min_m = s3_60min_mid(:, 3).*(38.52/546).^2;

s1_60min_t = s1_60min_tip(:, 3).*(38.52/546).^2;
s2_60min_t = s2_60min_tip(:, 3).*(38.52/546).^2;
s3_60min_t = s3_60min_tip(:, 3).*(38.52/546).^2;
...
%darks = [dark_blue; dark_blue; dark_blue;...
...
ylabel('<area> (µm^2)', 'FontSize', 17);
ax = gca;
ax.FontSize = 16;

% Add title and labels
title('2.5 µM, 37C', 'FontSize', 19);
xlim([0.5, 11.5]);
ylim([0, 45]);
hold on;
%breakyaxis([50, 150]);
